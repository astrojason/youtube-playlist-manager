...
The app version lives in `package.json` and must be displayed in the UI.
...
- The version in the UI must be a clickable link to `/changelog`. The `/changelog` route renders the git log — each entry shows the short hash and commit message (`git log --pretty=format:"%h %s" -n 50`). Implement as an Express route if not already present.
